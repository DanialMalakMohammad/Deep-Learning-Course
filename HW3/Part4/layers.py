...
class ConvLayer(object):
    ...
    def __call__(self, inputs):
        ...
        self.conv_filter = normal_initializer(
            shape=(self.kernel_size, self.kernel_size, self.input_filters, self.output_filters))
        ...
        #######################################################################
        self.conv_output = tf.nn.conv2d(inputs, self.conv_filter,
                                        strides=[1, self.kernel_stride, self.kernel_stride, 1],
                                        padding=self.kernel_padding)
        self.conv_output += self.conv_bias
        self.total_output = self.act(self.conv_output)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
class ConvPoolLayer(ConvLayer):
    def __init__(self, input_filters: object, output_filters: object, act: object,
                 kernel_size: object, kernel_stride: object, kernel_padding: object,
                 pool_size: object, pool_stride: object, pool_padding: object) -> object:
        # Calling ConvLayer constructor will store convolutional section config
        super().__init__(input_filters, output_filters, act,
                                            kernel_size, kernel_stride, kernel_padding)
    ...
    def _call(self, inputs):
        ...
        ##########################################################################
        self.pooling_output = tf.nn.max_pool(inputs,[1,self.pool_size,self.pool_size,1],[1,self.pool_stride,self.pool_stride,1],self.kernel_padding)
        ##########################################################################
        #                           END OF YOUR CODE                             #
        ##########################################################################
...
class DeconvLayer(object):
    ...
    def __call__(self, inputs):
        ...
        # Finally apply activation function and store it as self.total_output   #
        #########################################################################
        self.deconv_output = tf.nn.conv2d_transpose(inputs, filter=self.deconv_filter,output_shape=[10,output_height,output_width,self.output_filters],strides=[1,self.kernel_stride,self.kernel_stride,1],padding=self.kernel_padding)
