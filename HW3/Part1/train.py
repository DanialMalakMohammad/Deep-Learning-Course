...
def train(x_train, y_train, x_val, y_val, x_test, y_test, model, sess, log_file):
    ...
    for epoch in range(FLAGS.num_epoch):
        for batch in range(num_batch):
            ...
            #train_feed_dict.update( {x_train[batch_range]:y_train[batch_range]} )
            #train_feed_dict.update({(tf.placeholder, tf.placeholder): (x_train[batch_range], y_train[batch_range])})
            train_feed_dict.update({model.placeholders['batch_images']: x_train[batch_range],model.placeholders['batch_labels']: y_train[batch_range]})
        ...
        #train_feed_dict.update({[tf.placeholder,tf.placeholder]:[x_train , y_train]})
        train_feed_dict.update({model.placeholders['batch_images']: x_train,model.placeholders['batch_labels']: y_train})

        train_summary, train_loss, train_acc = sess.run((merged_summary,model._loss(),model._accuracy()),feed_dict=train_feed_dict)
        ...
        #val_feed_dict.update({x_val: y_val})
        val_feed_dict.update({model.placeholders['batch_images']: x_val,model.placeholders['batch_labels']: y_val})


        val_summary, val_loss = sess.run((merged_summary,model._loss()),val_feed_dict)
        ...
        #test_feed_dict.update({x_test: y_test})
        test_feed_dict.update({model.placeholders['batch_images']: x_test,model.placeholders['batch_labels']: y_test})

        test_summary = sess.run(merged_summary, test_feed_dict)
    ...
    #test_feed_dict.update({x_test: y_test})
    test_feed_dict.update({model.placeholders['batch_images']: x_test, model.placeholders['batch_labels']: y_test})
    test_loss, test_acc = sess.run((model._loss(),model._accuracy()), test_feed_dict)
