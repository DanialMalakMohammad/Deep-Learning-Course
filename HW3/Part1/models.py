...
class Dense(object):
    def __init__(self, num_hidden, weight_initializer, bias_initializer,
        ...
        #self.training = self.optimizer.apply_gradients(self.loss)
    ...
    def _build(self):
        for i in range(1, len(self.num_hidden)):
            ...
            layer =None
            layer = DenseLayer(input_dim=self.num_hidden[i-1],output_dim=self.num_hidden[i],act=act,weight_initializer=self.weight_initializer,bias_initializer=self.bias_initializer,stddev=self.stddev)
    ...
    def _loss(self):
        ...
        batch_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.placeholders['batch_labels'],logits=self.activations[-1]))


        # L2 regularization on weights

        l2_loss = FLAGS.weight_decay * (tf.nn.l2_loss(self.layers[0].vars['weight']) +tf.nn.l2_loss(self.layers[1].vars['weight']))

        # compute average of batch loss plus l2 loss
        avg_loss =batch_loss+l2_loss
    ...
    def _accuracy(self):
        ...
        batch_predictions =  tf.argmax(self.activations[-1],axis=1)
